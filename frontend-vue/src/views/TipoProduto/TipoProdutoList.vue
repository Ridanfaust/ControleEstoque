<template>
  ...
  <AppLayout>
    ...
    <div class="space-y-5 sm:space-y-6">
      ...
      <ComponentCard title="Lista de Tipos de Produto">
          ...
          <div class="max-w-full overflow-x-auto custom-scrollbar">
            <table class="min-w-full">
              ...
              <tr class="border-b border-gray-200 dark:border-gray-700">
                <th class="px-5 py-3 text-left w-3/4 sm:px-6">
              ...
              >
                ...
                <td class="px-5 py-4 sm:px-6 text-right">
                  <Button size="sm" variant="success" class="mr-2"
                          @click="editarTipoProduto(tipo.id)">
                    <font-awesome-icon icon="edit"/>
                  </Button>
                  <Button size="sm" variant="danger" @click="excluirTipoProduto(tipo.id)">
...
const currentPageTitle = ref("Tipos de Produto")
...
const {
  ...
  return await TipoProdutoService.listar(filters.descricao, page, size)
})
...
const novoCadastro = () => {
  router.push('/tipo-produto/cadastrar')
}

const editarTipoProduto = async (id) => {
  try {
    const response = await service.buscarPorId(id)
    const tipoProduto = response.data
    await router.push({
      path: `/tipo-produto/editar/${id}`,
      query: {data: JSON.stringify(tipoProduto)}
    })
  } catch (error) {
    showError(errorHandler.msgErro(error, 'Erro ao carregar tipo de produto'))
  }
}

const excluirTipoProduto = async (id) => {
  if (confirm('Tem certeza que deseja excluir este tipo de produto?')) {
    try {
      const response = await service.excluir(id)
      showSuccess(response.data.message || 'Tipo de produto excluído com sucesso!')
      buscarTiposProduto()
    } catch (error) {
      showError(errorHandler.msgErro(error, 'Erro ao excluir tipo de produto'))
    }
  }
}
